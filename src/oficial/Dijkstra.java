...
public class Dijkstra {
	...
	public List<Vertice> encontrarMenorCaminhoDijkstra(Grafo grafo, String id1,
			String id2) {
            
                Vertice v1 = grafo.encontrarVertice(id1);
                Vertice v2 = grafo.encontrarVertice(id2);
		...
		for (int i = 0; i < grafo.getVertices().size(); i++) {
			...
			// 9999("infinita")
			if (grafo.getVertices().get(i).getId()
					.equals(v1.getId())) {

				grafo.getVertices().get(i).setDist(0);

			} else {

				grafo.getVertices().get(i).setDist(9999);
		...
		while (!this.naoVisitados.isEmpty()) {
			...
			for (int i = 0; i < atual.getArestas().size(); i++) {
				...
				if (!vizinho.verificarVisita()) {
					...
					// distância
					if (vizinho.getDist() > (atual.getDist() + atual
							.getArestas().get(i).getPeso())) {

						vizinho.setDist(atual.getDist()
